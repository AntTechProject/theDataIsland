...
const MagicButton = ({ title, icon, position, handleClick, otherClasses }: {
    ...
    return (
        <button
            className="group/fire relative inline-flex h-12 w-full overflow-hidden rounded-lg p-[1px] focus:outline-none md:w-60 md:mt-10">
            <span
            ...
            <span
                ...
                {position === 'left' && (
                    <div className="transform group-hover/fire:scale-150 group-hover/fire:animate-pulse transition-transform duration-300">
                        {icon}
                    </div>
                )}
                {title}
